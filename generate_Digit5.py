...
def load_svhn(base_path):
    ...
    train_label = np.int64(svhn_train_data["y"].reshape(-1))
    test_label = np.int64(svhn_test_data["y"].reshape(-1))
...
def load_usps(base_path):
    ...
    train_label = np.int64(train_label.reshape(-1))
    train_label[train_label == 10] = 0
    usps_test = usps_dataset[1][0]
    test_label = usps_dataset[1][1]
    test_label = np.int64(test_label.reshape(-1))
    test_label[test_label == 10] = 0
...
def generate_Digit5(dir_path):
    ...
    for d in domains:
        ...
        train_dataloader.extend([(posi+pos, torch.utils.data.DataLoader(train_loader.dataset,
                                           batch_size=128,
                                           sampler=torch.utils.data.sampler.SubsetRandomSampler(list(random.sample(range(25000), 500))))) for pos in range(20)])
        test_dataloader.extend([(posi+pos, torch.utils.data.DataLoader(test_loader.dataset,
                                                                         batch_size=100,
                                                                         sampler=torch.utils.data.sampler.SubsetRandomSampler(list(random.sample(range(len(test_loader.dataset)), 100))))) for pos in range(20)])
        posi=posi+20
        # test_dataloader.append(torch.utils.data.DataLoader(test_loader.dataset,
        #                             batch_size=128,
        #                             shuffle=True))
    print(1)
...
def new_generate_shift_Digit5(dir_path,Dclient=20):
    if not os.path.exists(dir_path):
        os.makedirs(dir_path)
    ...
    for d in domains:
        ...
        for pos in range(Dclient):
            class_indices = random.sample(all_numbers, 2)
            # class_indices[0]=all_numbers[pos%10]
            subset_indices=[]
            subset_indices_test=[]
            num_samples = 200
            num_samples_test = 100
            for label in range(10):
                if label==class_indices[0] or label==class_indices[1]:
                    subset_indices.extend(list(random.sample([i for i in range(len(train_loader.dataset)) if train_loader.dataset.labels[i]==label],num_samples)))
                    subset_indices_test.extend(list(random.sample([i for i in range(len(test_loader.dataset)) if test_loader.dataset.labels[i]==label],num_samples_test)))
                else:
                    subset_indices.extend(list(random.sample(
                        [i for i in range(len(train_loader.dataset)) if train_loader.dataset.labels[i] == label],
                        13)))
                    subset_indices_test.extend(list(random.sample(
                        [i for i in range(len(test_loader.dataset)) if test_loader.dataset.labels[i] == label],
                        7)))
            # filtered_indices = [i for i in range(len(train_loader.dataset)) if train_loader.dataset.labels[i]==class_indices[0]]
            # filtered_indices1 = [i for i in range(len(train_loader.dataset)) if train_loader.dataset.labels[i]==class_indices[1]]
            # num_samples = 200
            # subset_indices=list(random.sample(filtered_indices,num_samples))
            # subset_indices1=list(random.sample(filtered_indices1,num_samples))
            # subset_indices.extend(subset_indices1)
            train_dataloader.extend([(posi+pos, torch.utils.data.DataLoader(train_loader.dataset,
                                               batch_size=32,
                                               sampler=torch.utils.data.sampler.SubsetRandomSampler(subset_indices))) ])

            # filtered_indices = [i for i in range(len(test_loader.dataset)) if
            #                     test_loader.dataset.labels[i] == class_indices[0]]
            # filtered_indices1 = [i for i in range(len(test_loader.dataset)) if
            #                      test_loader.dataset.labels[i] == class_indices[1]]
            # num_samples = 100
            # subset_indices = list(random.sample(filtered_indices, num_samples))
            # subset_indices1 = list(random.sample(filtered_indices1, num_samples))
            # subset_indices.extend(subset_indices1)
...
def generate_shift_Digit5(dir_path,Dclient=20):
    if not os.path.exists(dir_path):
        os.makedirs(dir_path)
    ...
    for d in domains:
        ...
        for pos in range(Dclient):
            ...
            num_samples = 250
            num_samples_test = 100
            subset_indices.extend(list(
                random.sample(
                    [i for i in range(len(train_loader.dataset)) if train_loader.dataset.labels[i] == class_indices[0]],
                    num_samples)))
            subset_indices.extend(list(
                random.sample(
                    [i for i in range(len(train_loader.dataset)) if train_loader.dataset.labels[i] == class_indices[1]],
                    num_samples)))
            subset_indices_test.extend(list(
                random.sample(
                    [i for i in range(len(test_loader.dataset)) if test_loader.dataset.labels[i] == class_indices[0]],
                    num_samples_test)))
            subset_indices_test.extend(list(
                random.sample(
                    [i for i in range(len(test_loader.dataset)) if test_loader.dataset.labels[i] == class_indices[1]],
                    num_samples_test)))
            train_dataloader.extend([(posi + pos, torch.utils.data.DataLoader(train_loader.dataset,
                                                                              batch_size=128,
